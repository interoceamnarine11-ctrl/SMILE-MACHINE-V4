...
import { GMergeStudioModal } from './GMergeStudioModal';
...
export const EmailSender: React.FC<EmailSenderProps> = ({ showToast, extractedLeads = [] }) => {
  ...
  // G-Merge Engine Studio & Personalization State
  const [isGMergeStudioOpen, setIsGMergeStudioOpen] = useState<boolean>(false);
  const [htmlEditorViewMode, setHtmlEditorViewMode] = useState<'editor' | 'split' | 'processed'>('editor');
  ...
  const handleStartSending = async () => {
    ...
    for (let i = 0; i < recipientsQueue.length; i++) {
      ...
      // Personalize subject & body for this specific recipient with G-Merge context and 1-based index
      const recipientWithContext = {
        ...recipient,
        senderName: headerConfig.senderName || smtpConfig.user?.split('@')[0] || 'Outreach Manager',
        senderOrganization: headerConfig.organization || 'Acme Global Enterprises',
        senderEmail: headerConfig.fromEmail || smtpConfig.user || 'sender@domain.com',
        unsubscribeUrl: headerConfig.listUnsubscribe || '#',
      };
      let personalizedSubject = substituteTemplateVariables(composerState.subject, recipientWithContext, i + 1);
      const rawBody = composerState.bodyMode === 'html' ? composerState.htmlContent : composerState.plainText;
      let personalizedBody = substituteTemplateVariables(rawBody, recipientWithContext, i + 1);
      let translationNote = '';
  ...
  // HTML Formatter / Beautifier helper for HTML editing
  const formatHtmlCode = (html: string): string => {
    ...
    for (const token of tokens) {
      ...
      } else if (trimmed.startsWith('<') && !trimmed.endsWith('/>') && !/^<(area|base|br|col|embed|hr|img|input|link|meta|param|source|track|wbr)/i.test(trimmed) && !trimmed.startsWith('<!')) {
        formatted += '\n' + tab.repeat(indent) + trimmed;
        indent++;
      } else {
  ...
  // Active recipient for live preview & G-Merge evaluation
  const previewRecipient = useMemo(() => {
    if (recipientsQueue.length > 0) {
      const safeIdx = Math.max(0, Math.min(previewRecipientIndex, recipientsQueue.length - 1));
      return recipientsQueue[safeIdx];
    }
    return {
      id: 'sample-alexander',
  ...
  }, [recipientsQueue, previewRecipientIndex]);

  // Backward compatibility alias
  const sampleRecipient = previewRecipient;

  // Live preview HTML text with tokens & full G-Merge language dynamically evaluated
  const previewHtml = useMemo(() => {
    const raw = composerState.bodyMode === 'html'
      ? composerState.htmlContent
      : `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; white-space: pre-wrap; font-size: 15px; line-height: 1.6; color: #1e293b; padding: 24px; background-color: #ffffff;">${composerState.plainText}</div>`;
    const recipientWithSender = {
      ...previewRecipient,
      senderName: headerConfig.senderName || smtpConfig.user?.split('@')[0] || 'Outreach Manager',
      senderOrganization: headerConfig.organization || 'Acme Global Enterprises',
      senderEmail: headerConfig.fromEmail || smtpConfig.user || 'outbox@system.local',
      unsubscribeUrl: headerConfig.listUnsubscribe || '#',
    };
    return substituteTemplateVariables(raw, recipientWithSender, (previewRecipientIndex || 0) + 1);
  }, [composerState, previewRecipient, headerConfig, smtpConfig.user, previewRecipientIndex]);

  // Evaluated compiled raw HTML source code (for HTML code inspection & export)
  const processedHtmlSource = useMemo(() => {
    ...
    return substituteTemplateVariables(composerState.htmlContent, recipientWithSender, (previewRecipientIndex || 0) + 1);
  }, [composerState.htmlContent, previewRecipient, headerConfig, smtpConfig.user, previewRecipientIndex]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* --- TAB 1: COMPOSE & LIVE PREVIEW (WITH G-MERGE ENGINE & HTML STUDIO) --- */}
      {/* ========================================================================= */}
      {activeSubTab === 'compose' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Left Column: Composer Editor & G-Merge Studio */}
          <div className="lg:col-span-7 space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-4">
              {/* Template Selector Bar & G-Merge Studio Launchers */}
              <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-gray-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setIsGMergeStudioOpen(true)}
                    className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-md shadow-indigo-900/30"
                    title="Launch interactive G-Merge Engine Studio, live expression tester, variable inspector, and syntax manual"
                  >
                    <Terminal className="w-3.5 h-3.5" />
                    <span>G-Merge Studio &amp; Tester</span>
                  </button>
                </div>
              </div>

              {/* Subject Line with G-Merge Support */}
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] text-gray-400 font-mono">
                    G-Merge: <code className="text-indigo-400 font-bold">[[-Company-]]</code>, <code className="text-teal-400 font-bold">[[-FirstName-:Executive]]</code>
                  </span>
                </div>
                <div className="relative">
                  <input
                    ...
                    placeholder="e.g. Collaboration update for [[-Company-]] [[if Tier='Gold']](VIP)[[!endif]]"
                    className="w-full px-3.5 py-2.5 bg-gray-950 border border-gray-700 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-all font-medium"
                  />
                </div>
              </div>

              {/* Format Toggle: Plain Text vs HTML Template & HTML View Controls */}
              <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                ...
                {composerState.bodyMode === 'html' && (
                  <div className="flex items-center gap-1 bg-gray-950 p-1 rounded-xl border border-gray-800 text-xs">
                    <button
                      type="button"
                      onClick={() => setHtmlEditorViewMode('editor')}
                      className={`px-2.5 py-1 rounded-lg font-semibold transition-all ${
                        htmlEditorViewMode === 'editor' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-400 hover:text-white'
                      }`}
                    ...
                    <button
                      type="button"
                      onClick={() => setHtmlEditorViewMode('split')}
                      className={`px-2.5 py-1 rounded-lg font-semibold transition-all flex items-center gap-1 ${
                        htmlEditorViewMode === 'split' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-400 hover:text-white'
                      }`}
                      title="Side-by-side Code and Rendered Preview"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setHtmlEditorViewMode('processed')}
                      className={`px-2.5 py-1 rounded-lg font-semibold transition-all flex items-center gap-1 ${
                        htmlEditorViewMode === 'processed' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-400 hover:text-white'
                      }`}
                      title="View compiled HTML with all G-Merge tags evaluated"
                    >
              ...
              {/* HTML Snippet & Formatting Action Toolbar (When in HTML mode) */}
              {composerState.bodyMode === 'html' && (
                <div className="flex flex-wrap items-center justify-between gap-1.5 bg-gray-950/80 p-2 rounded-xl border border-gray-800 text-xs">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const beautified = formatHtmlCode(composerState.htmlContent);
                        setComposerState(prev => ({ ...prev, htmlContent: beautified }));
                        showToast('Formatted and beautified HTML code!');
                      }}
                      className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded-lg border border-gray-700 text-[11px] font-medium transition-all"
                      title="Automatically indent tags and organize HTML structure"
                    >
                      Beautify Code
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const btnSnippet = `\n<div style="margin: 28px 0; text-align: center;">\n  <a href="https://[[-Domain-]]" style="background-color: #2563eb; color: #ffffff; text-decoration: none; padding: 12px 28px; border-radius: 6px; font-weight: 700; font-size: 14px; display: inline-block;">Schedule 15-Min Meeting</a>\n</div>\n`;
                        setComposerState(prev => ({ ...prev, htmlContent: prev.htmlContent + btnSnippet }));
                        showToast('Inserted CTA button into HTML!');
                      }}
                      className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-cyan-300 rounded-lg border border-gray-700 text-[11px] font-medium transition-all"
                    >
                      + CTA Button
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const tableSnippet = `\n<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin: 20px 0;">\n  <tr>\n    <td width="48%" style="padding: 16px; background-color: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; vertical-align: top;">\n      <strong style="color: #0f172a; font-size: 14px;">High-Deliverability Architecture</strong>\n      <p style="margin: 8px 0 0 0; font-size: 13px; color: #475569; line-height: 1.5;">Clean SPF, DKIM, and DMARC verified outbound nodes for [[-Company-]].</p>\n    </td>\n    <td width="4%"></td>\n    <td width="48%" style="padding: 16px; background-color: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; vertical-align: top;">\n      <strong style="color: #0f172a; font-size: 14px;">Dedicated Enterprise SLA</strong>\n      <p style="margin: 8px 0 0 0; font-size: 13px; color: #475569; line-height: 1.5;">Direct escalation contact with guaranteed response windows.</p>\n    </td>\n  </tr>\n</table>\n`;
                        setComposerState(prev => ({ ...prev, htmlContent: prev.htmlContent + tableSnippet }));
                        showToast('Inserted 2-column responsive section!');
                      }}
                      className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-teal-300 rounded-lg border border-gray-700 text-[11px] font-medium transition-all"
                    >
                      + 2-Col Table
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const footerSnippet = `\n<div style="margin-top: 32px; padding: 20px 24px; background-color: #f8fafc; border-top: 1px solid #e2e8f0; font-size: 12px; color: #64748b; text-align: center;">\n  <p style="margin: 0 0 6px 0;">This email was sent to <strong>[[-Email-]]</strong> regarding your affiliation with <strong>[[-Company-]]</strong>.</p>\n  <p style="margin: 0;"><a href="[[unsubscribe_url]]" style="color: #64748b; text-decoration: underline;">Unsubscribe from outreach</a> &bull; [[sender_organization]]</p>\n</div>\n`;
                        setComposerState(prev => ({ ...prev, htmlContent: prev.htmlContent + footerSnippet }));
                        showToast('Inserted compliant legal footer & unsubscribe!');
                      }}
                      className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-amber-300 rounded-lg border border-gray-700 text-[11px] font-medium transition-all"
                    >
                      + Legal Footer
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(composerState.htmlContent);
                        showToast('Copied HTML template to clipboard!');
                      }}
                      className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white rounded-lg border border-gray-700 text-[11px] font-medium transition-all flex items-center gap-1"
                    >
              ...
              <div>
                {composerState.bodyMode === 'plain' ? (
                  <textarea
                    ...
                    placeholder="Compose plain text email using G-Merge statements like [[-FirstName-:Customer]]..."
                    className="w-full p-3.5 bg-gray-950 border border-gray-700 rounded-xl text-sm text-gray-200 font-mono leading-relaxed focus:outline-none focus:border-indigo-500 transition-all resize-y shadow-inner"
                  />
                ) : htmlEditorViewMode === 'split' ? (
                  /* Split View: Code Editor on Left, Live Render on Right */
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 min-h-[360px]">
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        className="w-full h-[360px] p-3 bg-gray-950 border border-gray-700 rounded-xl text-xs text-emerald-300 font-mono leading-relaxed focus:outline-none focus:border-purple-500 transition-all resize-none shadow-inner"
                        spellCheck={false}
                      />
                    </div>
                    <div className="space-y-1">
                      <div className="text-[11px] text-gray-400 font-bold uppercase tracking-wider flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-indigo-400">Recipient #[[(previewRecipientIndex || 0) + 1]]</span>
                      </div>
                      <div className="w-full h-[360px] bg-white rounded-xl overflow-hidden border border-gray-700 shadow-inner">
                        <iframe
                          title="Split Preview Frame"
                          srcDoc={previewHtml}
                ...
                ) : htmlEditorViewMode === 'processed' ? (
                  /* Processed HTML Code View */
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs text-gray-400">
                      <span className="font-semibold text-emerald-400 flex items-center gap-1.5">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        Compiled Output for: {previewRecipient.name || previewRecipient.email}
                      </span>
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(processedHtmlSource);
                          showToast('Copied compiled HTML source to clipboard!');
                        }}
                        className="px-2.5 py-1 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded-lg text-xs flex items-center gap-1 border border-gray-700 transition-all"
                      >
                        <Copy className="w-3 h-3" />
                        Copy Compiled HTML
                      </button>
                    </div>
                    <pre className="w-full p-3.5 bg-gray-950 border border-gray-700 rounded-xl text-xs text-emerald-300 font-mono leading-relaxed max-h-[360px] overflow-auto whitespace-pre-wrap shadow-inner">
                      {processedHtmlSource || '(Empty HTML output)'}
                    </pre>
                  </div>
                ) : (
                  /* Standard Full-Width Code Editor */
                  <div className="space-y-2">
                    <textarea
                      rows={14}
                      value={composerState.htmlContent}
                      onChange={(e) => setComposerState(prev => ({ ...prev, htmlContent: e.target.value }))}
                      placeholder="Paste raw HTML here (e.g. <table>...</table>)... Supports full [[...]] G-Merge syntax!"
                      className="w-full p-3.5 bg-gray-950 border border-gray-700 rounded-xl text-xs text-emerald-300 font-mono leading-relaxed focus:outline-none focus:border-purple-500 transition-all resize-y shadow-inner"
                      spellCheck={false}
                    />
              ...
              {/* Comprehensive G-Merge Personalization Studio & Quick Injector */}
              <div className="pt-3 border-t border-gray-800 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="p-1 bg-indigo-600/20 text-indigo-400 rounded-lg border border-indigo-500/40">
                      <Terminal className="w-3.5 h-3.5" />
                    </div>
                    <span className="text-xs font-bold text-white tracking-wide">
                      G-Merge Personalization Studio:
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>Open Live Tester &amp; Evaluator</span>
                    </button>
                  </div>
                </div>

                {/* Categorized Token Injection Buttons */}
                <div className="space-y-2 text-xs">
                  {/* Category 1: Predefined Variables */}
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] uppercase font-bold text-gray-400 w-24 shrink-0">Predefined:</span>
                    {[
                      { token: '[[-FirstName-]]', title: 'Recipient first name' },
                      { token: '[[-LastName-]]', title: 'Recipient last name' },
                      { token: '[[-FullName-]]', title: 'Recipient full name' },
                      { token: '[[-Company-]]', title: 'Recipient company' },
                      { token: '[[-Domain-]]', title: 'Company domain' },
                      { token: '[[-Email-]]', title: 'Email address' },
                      { token: '[[-Now-]]', title: 'Current date and time' },
                      { token: '[[-Index-]]', title: 'Message 1-based sequential index' },
                      { token: '[[-Recipient-]]', title: 'Full compound recipient' },
                    ].map(t => (
                      <button
                        key={t.token}
                        type="button"
                        ...
                        className="px-2 py-0.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-indigo-500/50 rounded text-indigo-300 font-mono text-[11px] transition-all"
                        title={t.title}
                      >
                        +{t.token}
                      </button>
                  ...
                  {/* Category 2: Smart Defaults & Date Formatting */}
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] uppercase font-bold text-gray-400 w-24 shrink-0">Defaults &amp; Dates:</span>
                    {[
                      { token: '[[-FirstName-:Valued Partner]]', title: 'Fallback if first name is empty' },
                      { token: '[[City:your region]]', title: 'Fallback for city column' },
                      { token: '[[-Now-:MM D, yy]]', title: 'e.g. September 25th, 2026' },
                      { token: '[[-Now-:m/d/y]]', title: 'e.g. 9/25/26' },
                      { token: '[[-Now- + 7:MM D, yy]]', title: 'Date 7 days in future' },
                      { token: '[[-Now-:~@]]', title: 'Regional date and time' },
                    ].map(t => (
                      <button
                        key={t.token}
                        type="button"
                        ...
                        className="px-2 py-0.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-teal-500/50 rounded text-teal-300 font-mono text-[11px] transition-all"
                        title={t.title}
                      >
                        +{t.token}
                      </button>
                  ...
                  {/* Category 3: Conditionals, Data Switching & Math */}
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] uppercase font-bold text-gray-400 w-24 shrink-0">Logic &amp; Math:</span>
                    {[
                      { token: "[[if gender=1]]Sir[[else]]Ma'am[[endif]]", title: "Conditional IF/ELSE" },
                      { token: "[[Status|1|Active|2|Pending|Under Review]]", title: "Data switching lookup" },
                      { token: "[[convert_lower_case(-Email-)]]", title: "Lowercase function call" },
                      { token: "[[extract_first_name(-FullName-)]]", title: "First name extractor" },
                      { token: "[[2 * 3.14159 * radius]]", title: "Mathematical calculation" },
                    ].map(t => (
                      <button
                        key={t.token}
                        type="button"
                        ...
                        className="px-2 py-0.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-amber-500/50 rounded text-amber-300 font-mono text-[11px] transition-all"
                        title={t.title}
                      >
                        +{t.token.slice(0, 24)}...
                      </button>
                ...
                {/* Auto-Translate Email by Target Country */}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 leading-relaxed">
                        Translates subject and body to the recipient's country language, preserving HTML formatting, custom columns, and G-Merge statements.
                      </p>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 shadow-xl space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-gray-800">
                ...
                {/* Recipient Switcher Controls for G-Merge Testing */}
                <div className="flex items-center gap-2">
                  {recipientsQueue.length > 1 && (
                    <div className="flex items-center gap-1 bg-gray-950 px-2 py-1 rounded-lg border border-gray-800 text-[11px] text-gray-300">
                      <button
                        type="button"
                      ...
                      <span className="font-mono text-indigo-300">
                        {previewRecipientIndex + 1}/{recipientsQueue.length}
                      </span>
                      <button
                        type="button"
                        onClick={() => setPreviewRecipientIndex(prev => Math.min(recipientsQueue.length - 1, prev + 1))}
                        disabled={previewRecipientIndex >= recipientsQueue.length - 1}
                        className="p-0.5 hover:text-white disabled:opacity-30"
              ...
              {/* Simulated Email Client Header with evaluated G-Merge data */}
              <div className="p-3 bg-gray-950 rounded-xl border border-gray-800 text-xs space-y-1">
                ...
                <div className="text-gray-400 truncate flex items-center justify-between">
                  <div className="truncate">
                    <strong>To:</strong> <span className="text-blue-300 font-medium">{previewRecipient.name ? `"${previewRecipient.name}" ` : ''}&lt;{previewRecipient.email}&gt;</span>
                  </div>
                  {previewRecipient.company && (
                    <span className="text-[10px] text-gray-400 bg-gray-900 px-1.5 py-0.5 rounded border border-gray-800 shrink-0">
                      {previewRecipient.company}
                    </span>
                  )}
                </div>
                ...
                <div className="text-gray-400 truncate pt-1 border-t border-gray-800/80">
                  <strong>Subject:</strong> <span className="text-white font-semibold">{substituteTemplateVariables(composerState.subject, previewRecipient, (previewRecipientIndex || 0) + 1) || '(No Subject)'}</span>
                </div>
              </div>
      ...
      {/* G-Merge Engine Studio, Live Evaluator & Documentation Modal */}
      <GMergeStudioModal
        isOpen={isGMergeStudioOpen}
        onClose={() => setIsGMergeStudioOpen(false)}
        onInsertToEditor={(snippet) => {
          if (composerState.bodyMode === 'plain') {
            setComposerState(prev => ({ ...prev, plainText: prev.plainText ? `${prev.plainText}\n\n${snippet}` : snippet }));
          } else {
            setComposerState(prev => ({ ...prev, htmlContent: prev.htmlContent ? `${prev.htmlContent}\n\n${snippet}` : snippet }));
          }
          showToast('Inserted G-Merge snippet into email body!');
        }}
        showToast={showToast}
      />
    </div>
