...
import { User, Building2, Send, Globe, CheckCircle2, ArrowRight } from 'lucide-react';
...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ 
  ...
  const [nameTypeFilter, setNameTypeFilter] = useState<'all' | 'personal' | 'role'>('all');
    ...
    return results.filter(result => {
      const companyMatch = companyFilter
        ? result.companyName?.toLowerCase().includes(companyFilter.toLowerCase()) || 
          result.email.toLowerCase().includes(companyFilter.toLowerCase()) ||
          (result.firstName && result.firstName.toLowerCase().includes(companyFilter.toLowerCase())) ||
          (result.lastName && result.lastName.toLowerCase().includes(companyFilter.toLowerCase()))
        : true;
      ...
      const nameTypeMatch =
        nameTypeFilter === 'all'
          ...
          ? !result.isRoleBased && Boolean(result.firstName || result.lastName)
          : Boolean(result.isRoleBased);

      return companyMatch && countryMatch && validityMatch && nameTypeMatch;
    });
  }, [results, companyFilter, countryFilter, validityFilter, nameTypeFilter]);

  // Reset to page 1 whenever filters or results change
  useEffect(() => {
    setCurrentPage(1);
  }, [companyFilter, countryFilter, validityFilter, nameTypeFilter, results.length]);
  ...
  const areFiltersActive = companyFilter || countryFilter || validityFilter !== 'all' || nameTypeFilter !== 'all';
  ...
  const handleExportCSV = () => {
    const header = "Email,FirstName,LastName,FullName,CompanyName,Website,Country,AccountType,IsValid\n";
    const csvContent = filteredResults.map(r => {
      ...
      const full = r.fullName ? `"${r.fullName.replace(/"/g, '""')}"` : (r.isRoleBased ? '"No Name (Role Account)"' : '');
      const accType = r.isRoleBased ? '"Role/Generic (info, sales, orders)"' : '"Personal Contact"';
      return [r.email, fn, ln, full, company, r.sourceUrl, r.country || 'N/A', accType, String(r.isValid)].join(',');
    }).join('\n');
  ...
  const handleExportExcel = () => {
    const rows = filteredResults.map(r => ({
      ...
      "Full Name": r.fullName || (r.isRoleBased ? "No Name (Role Account)" : ""),
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="overflow-auto custom-scrollbar flex-grow">
            ...
            <thead className="bg-gray-900/95 sticky top-0 z-10 backdrop-blur-md border-b border-gray-700 shadow-sm">
                <tr>
                <th scope="col" className="py-3 pl-6 pr-4 text-xs font-bold uppercase tracking-wider text-gray-400 w-[25%]">Email Address & Type</th>
                <th scope="col" className="px-3 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 w-[12%]">First Name</th>
                <th scope="col" className="px-3 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 w-[12%]">Last Name</th>
                <th scope="col" className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 w-[24%]">Company & Website</th>
                <th scope="col" className="px-3 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 w-[12%]">Country</th>
                <th scope="col" className="px-2 py-3 text-center text-xs font-bold uppercase tracking-wider text-gray-400 w-[8%]">Status</th>
                <th scope="col" className="py-3 pr-6 text-right text-xs font-bold uppercase tracking-wider text-gray-400 w-[7%]">Action</th>
                </tr>
            </thead>
            <tbody className="divide-y divide-gray-800/50 bg-transparent">
                ...
                <tr key={`${item.email}-${index}`} className="hover:bg-gray-700/40 odd:bg-gray-800/20 transition-colors duration-150 group">
                    <td className="py-3 pl-6 pr-4 align-top truncate">
                        <div className="flex flex-col">
                            <span className="font-mono text-xs sm:text-sm text-blue-300 font-medium selection:bg-blue-500/30 truncate" title={item.email}>{item.email}</span>
                            <div className="flex items-center gap-1.5 mt-1">
                                {item.isRoleBased ? (
                                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-400 bg-amber-950/70 border border-amber-800/60 px-1.5 py-0.2 rounded" title="Generic department / role address (info, sales, order, etc.)">
                                        <Building2 className="w-2.5 h-2.5" />
                                        Role / Generic
                                    </span>
                                ) : (
                                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-400 bg-emerald-950/70 border border-emerald-800/60 px-1.5 py-0.2 rounded" title="Personal named mailbox">
                    ...
                    <td className="px-3 py-3.5 align-top">
                        {item.isRoleBased && !item.firstName ? (
                            <span className="text-[11px] text-gray-500 italic" title="No personal name (generic mailbox)">— No Name</span>
                        ) : item.firstName ? (
                            <span className="text-xs font-bold text-emerald-300">{item.firstName}</span>
                        ) : (
                            <span className="text-xs text-gray-500">—</span>
                        )}
                    </td>
                    <td className="px-3 py-3.5 align-top">
                        {item.isRoleBased && !item.lastName ? (
                            <span className="text-[11px] text-gray-500 italic" title="No personal name (generic mailbox)">— No Name</span>
                        ) : item.lastName ? (
                            <span className="text-xs font-bold text-emerald-300">{item.lastName}</span>
                        ) : (
                            <span className="text-xs text-gray-500">—</span>
                        )}
                    </td>
                    <td className="px-4 py-3 align-top">
                        <div className="flex flex-col">
                            ...
                            <a 
                                ...
                                className="text-[11px] text-gray-500 group-hover:text-blue-400 transition-colors flex items-center gap-1 truncate max-w-full"
                    ...
                    <td className="px-3 py-3.5 text-xs text-gray-300 align-top truncate font-medium">
                    ...
                    <td className="px-2 py-3.5 text-center align-top">
                        {item.isValid ? 
                            <span className="inline-flex items-center rounded bg-green-900/40 px-2 py-0.5 text-[10px] font-bold text-green-400 ring-1 ring-inset ring-green-500/30 uppercase tracking-wide">Valid</span> :
                            <span className="inline-flex items-center rounded bg-red-900/40 px-2 py-0.5 text-[10px] font-bold text-red-400 ring-1 ring-inset ring-red-500/30 uppercase tracking-wide">Invalid</span>
                        }
                    </td>
                        ...
                        <div className="flex items-center justify-end gap-1">
                            ...
                            {onSendToEmailSender && (
                                <button
                                    ...
                                    className="p-2 rounded-md bg-blue-900/40 hover:bg-blue-600 text-blue-300 hover:text-white transition-all duration-200"
                                    title="Send this contact directly to Email Sender"
                                >
        ...
        {results.length > 0 && (
            <div className="flex flex-wrap items-center gap-3 pt-2">
                ...
                <select
                    ...
                    className="px-3 py-1.5 bg-gray-900/50 border border-gray-600 rounded-md focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition text-sm text-gray-300"
                >
                    <option value="all">All Contacts & Roles</option>
                    <option value="personal">👤 Personal Names Only</option>
                    <option value="role">🏢 Role Accounts (Info, Sales, Orders...)</option>
                </select>
